...
class B1CCO_WC_Smart_Coupons
{
	...
	public function custom_update_order($request)
	{
			...
			foreach ($data['coupon_lines'] as $coupon_line) {
				...
				if (isset($coupon_code)) {
					...
					if (empty($max_discount)) {
						$max_discount = (float) get_post_meta($coupon_id, 'wc_sc_max_discount', true);
					}

					// Apply coupon with max discount if _wt_max_discount or wc_sc_max_discount is set
		...
		$converted_tax_lines = $this->prepare_tax_lines_data($order);
	...
	public function prepare_coupon_lines($order)
	{
		...
		foreach ($coupon_lines as $coupon_code) {
			...
			foreach ($coupon_payload['meta_data'] as $meta) {
				...
				if (isset($meta_array['key']) && $meta_array['key'] === 'wc_sc_max_discount') {
					$max_discount = $meta_array['value'];
					break;
				}
			}
			...
			$meta_value = array(
				...
				'amount' => ($max_discount !== null && $max_discount !== 0 && $max_discount !== '0' && $max_discount !== '') ? $max_discount :
					$applied_discount,
			...
			$coupon_lines_data[] = array(
				'code' => $coupon_payload['code'],
				'discount' => ($max_discount !== null && $max_discount !== 0 && $max_discount !== '0' && $max_discount !== '') ? $max_discount :
					$applied_discount,
				'meta_data' => array(
					array(
						'value' => $meta_value,
	...
	public function prepare_tax_lines_data($order)
	{
		// Validate that $order is an instance of WC_Order
		...
		// Get tax totals and validate they are iterable
		$tax_totals = $order->get_tax_totals();
		if (empty($tax_totals) || !is_iterable($tax_totals)) {
			return [];
		}

		foreach ($tax_totals as $tax_total) {
			// Validate required properties of the tax total object
			if (
				empty($tax_total->rate_id) ||
				!isset($tax_total->id, $tax_total->label, $tax_total->amount, $tax_total->is_compound)
			) {
				continue; // Skip invalid tax total entries
			}

			$rate_id = (int) $tax_total->rate_id;
			$shipping_tax_total = 0;

			// Get shipping tax total for the current tax rate
			$tax_items = $order->get_items('tax');
			if (is_iterable($tax_items)) {
				foreach ($tax_items as $tax_item) {
					if ((int) $tax_item->get_rate_id() === $rate_id) {
						$shipping_tax_total = (float) $tax_item->get_shipping_tax_total();
						break;
					}
				}
			}

			// Safely get the tax rate code and percentage
			$rate_code = \WC_Tax::get_rate_code($rate_id) ?? '';
			$rate_percent = \WC_Tax::get_rate_percent_value($rate_id) ?? '';

			$converted_tax_lines[] = [
				'id' => $tax_total->id,
				'rate_code' => sanitize_text_field($rate_code),
				'rate_id' => $rate_id,
				'label' => sanitize_text_field($tax_total->label),
				'compound' => (bool) $tax_total->is_compound,
				'tax_total' => strval($tax_total->amount),
				'shipping_tax_total' => strval($shipping_tax_total),
				'rate_percent' => $rate_percent,
			];
		}

		return $converted_tax_lines;
	}
}

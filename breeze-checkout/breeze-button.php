...
define('B1CCO_PLUGIN_STATUS_HOOK_URL', '<plugin_status_url>'); // https://eomew9dw775ge9m.m.pipedream.net
define('B1CCO_ORDER_REFUND_WEBHOOK_URL', esc_url('https://api.breeze.in/order/refund/webhook/' . get_option('breeze_refund_shop_id', 'CHANGE')));
define('B1CCO_REFUND_WEBHOOK_SIGN_KEY', sanitize_text_field(get_option('breeze_refund_api_signkey', '48304eb47085c8996815aacaf1862f4352d3788f507dbd4d4dd51bf86ab686e33c14b1020b85027321443e7e80cbd251')));

// Update options
update_option('b1cco_btn_enable_order_refund', (get_option('breeze_enable_refund', 'no') === 'yes'));
...
require_once __DIR__ . '/admin/configuration/main.php';
...
if (!class_exists('B1CCO_Checkout')) {
	...
	class B1CCO_Checkout
	{
		...
		{
			$file_version = get_option('breeze_file_version', '0.1');

			define('B1CCO_FILE_VER', $file_version);
		...
		{
			$ghost_mode = (get_option('breeze_enable_ghost_mode', 'no') === 'yes') ? 'true' : 'false';
			$ga_enabled = (get_option('breeze_enable_google_tracker', 'no') === 'yes') ? 'true' : 'false';
			$fbp_enabled = (get_option('breeze_enable_fb_tracker', 'no') === 'yes') ? 'true' : 'false';

			$ga_measurement_id = esc_attr(get_option('breeze_google_measurement_id', 'GA4'));
			$mid = esc_attr(get_option('breeze_merchant_id', 'CHANGE'));
			...
			echo '<script 
					...
					data-enable-ga=' . esc_attr($ga_enabled) . '
					data-enable-fbp=' . esc_attr($fbp_enabled) . '
					data-emit-tracker-events="true" 
					data-ga4-measurement-id=' . esc_attr($ga_measurement_id) . '
					data-merchantid=' . esc_attr($mid) . '
					data-ga-version="new"
					ghost-mode=' . esc_attr($ghost_mode) . '>
				</script>';
		}
